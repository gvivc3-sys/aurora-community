...
function cleanName(filename: string): string {
  return filename
    ...
    .replace(/-\d{10,}$/, "")          // strip timestamp suffix
    .replace(/\bPDF\b/gi, "")          // strip trailing "PDF"
...
export default async function LibraryPage() {
  ...
  let files: string[] = [];
  try {
    files = readdirSync(guidesDir).filter((f) => !f.startsWith("."));
  } catch {
    files = [];
  }
      ...
      <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 sm:py-12">
        ...
        <div className="mt-8 space-y-3">
          ...
          {files.map((filename) => {
            ...
            const href = `/guides/${encodeURIComponent(filename)}`;
            ...
            return (
              ...
              >
                {/* File icon */}
                <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${type === "pdf" ? "bg-rose-50 text-rose-500" : type === "zip" ? "bg-fuchsia-50 text-fuchsia-500" : "bg-warm-100 text-warm-500"}`}>
                  {type === "pdf" && (
